  ...
  conn.createChannel(function(err, ch) {
    ...
    ch.assertQueue('', {exclusive: true}, function(err, q) {
        ...
        es_states.select_last(json_message.data.client, function (hits) {
          ...
          if(hits == null) {
            ...
            es_states.insert(json_message.data.client, 
                             json_message.data.state, 
                             json_message.data.trans_id);            
          } else if (hits[0]._source.event == 'alerted_up') {
      ...
      if(json_message.data.state == 'up') {
        logger.log("up state detected");
        es_states.select_last(json_message.data.client, function (hits) {
          //logger.log("Message.state:down. Last Event Was: "+hits[0]._source.event);
          if(hits == null) {
            logger.log("inserting, new record...");
            es_states.insert(json_message.data.client, json_message.data.state,json_message.data.trans_id);
          } else if (hits[0]._source.event == 'alerted_down') {
            logger.log("inserting, last record was alerted state...");
            es_states.insert(json_message.data.client, 
                             json_message.data.state, 
                             json_message.data.trans_id);
          }
